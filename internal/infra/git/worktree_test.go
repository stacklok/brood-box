...
func TestWorktreeProcessor_ExternalWorktree(t *testing.T) {
	...
	// Verify config exists (and is sanitized — covered in separate test).
...
func TestWorktreeProcessor_ConfigIsSanitized(t *testing.T) {
	t.Parallel()

	// Set up main repo with a config containing credentials.
	mainRepo := t.TempDir()
	mainGitDir := filepath.Join(mainRepo, ".git")
	require.NoError(t, os.MkdirAll(mainGitDir, 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(mainGitDir, "HEAD"), []byte("ref: refs/heads/main\n"), 0o644))
	...
	// Config with credential section and URL containing user:token.
	...
	// Read the config that was written to the snapshot.
	sanitizedConfig, err := os.ReadFile(filepath.Join(snapshot, ".git", "config"))
	require.NoError(t, err)

	configStr := string(sanitizedConfig)

	// Credential section should be stripped.
	assert.NotContains(t, configStr, "[credential]")
	assert.NotContains(t, configStr, "helper = store")

	// URL should have credentials stripped.
	assert.NotContains(t, configStr, "user:token")

	// Core section should still be present.
	assert.Contains(t, configStr, "[core]")
	assert.Contains(t, configStr, "bare = false")

	// Remote section should still be present (with sanitized URL).
	assert.Contains(t, configStr, `[remote "origin"]`)
	assert.Contains(t, configStr, "github.com/org/repo.git")
}
