...
func (w *WorktreeProcessor) Process(_ context.Context, originalPath, snapshotPath string) (*workspace.PostProcessResult, error) {
	...
	// Defense-in-depth: a crafted .git file could point anywhere on the host.
	...
	// 7d: Copy config (sanitized), packed-refs, and info/exclude from main repo .git/.
	configData, err := os.ReadFile(filepath.Join(mainGitDir, "config"))
	if err != nil {
		if !errors.Is(err, fs.ErrNotExist) {
			return nil, fmt.Errorf("reading git config from main repo: %w", err)
		}
		w.logger.Debug("no config file in main repo git dir")
	} else {
		sanitized := SanitizeConfig(string(configData))
		if err := os.WriteFile(filepath.Join(dotGitPath, "config"), []byte(sanitized), 0o644); err != nil {
...
func copyFile(src, dst string) error {
	...
	// Preserve source file permissions, defaulting to 0o644.
